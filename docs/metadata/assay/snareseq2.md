...
The attributes shown here have been colleced for SNARE-seq2 data, available from the [HuBMAP Search and Entity APIs](/apis) at ```Dataset.metadata.<attribute>```.<br />
These attributes are harmonized across all versions of the SNARE-seq2 metadata specifications. See the latest version of the [SNARE-seq2 Metadata Specifications](https://hubmapconsortium.github.io/ingest-validation-tools/snareseq2/current/) for the schema and directory structure needed when ingesting Segmentation Mask metadata.
&nbsp;

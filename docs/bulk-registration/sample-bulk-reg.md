...
| organ_type | This field is required only if the sample_category is specified as `organ`.  This is the HuBMAP defined code for the organ that this registration represents (example, `BR` for Brain or `LK` for Left Kidney) from the [JSON Array of Organ Typs](https://ontology.api.hubmapconsortium.org/organs/by-code?application_context=HUBMAP). |

...
|-
  ...
  | raw/lightpath_configuration.tsv$ |  | This file documents the light path setttings not captured in the OME-TIFF - will be deprecated upon the implementation of MicroMeta. |
  | raw/fingerprint_layout.tsv$ |  | Table that includes a dictionary for channel to moiety which is determined from spectral clustering and may include lipids, protein, etc. This file is specific to hyper-spectral imaging. |

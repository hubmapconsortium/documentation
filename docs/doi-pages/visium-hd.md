...
schema_items: |-
  ...
  | Spot size value <span class="requiredMark">*</span> | <i class="fa-solid fa-hashtag" title="Numeric" aria-label="Numeric"></i> | The area of a spot used in assays where spots define discrete capture areas. Example: 2375.9 |  |
  | Spot size unit <span class="requiredMark">*</span> | <i class="fa-solid fa-circle-nodes" title="Allowable Value" aria-label="Allowable Value"></i> | The unit of measurement for the spot size value. If no spot size value is specified, this field may be left blank. Example: um^2 | ```mm^2``` ```um^2``` |
  ...
  | Spot spacing value <span class="requiredMark">*</span> | <i class="fa-solid fa-hashtag" title="Numeric" aria-label="Numeric"></i> | The approximate center-to-center distance between capture spots, also known as inter-spot distance, spot resolution, or pit size. Example: 100 |  |
...
|-
  ...
  | lab_processed/spaceranger/ |  | Output from the 10X Genomics SpaceRanger pipeline |
  | lab_processed/spaceranger/filtered_feature_bc_matrix.h5$ |  | HDF5 file containing called cell x gene matrix |
  | lab_processed/spaceranger/raw_feature_bc_matrix.h5$ |  | HDF5 file containing unfiltered cell x gene matrix |
  | lab_processed/spaceranger/feature_slice.h5$ | ✓ | HDF5 file containing Visium HD gene expression data, structured for easy and efficient retrieval of 2 µm resolution "image" slices for single or multiple genes. The data can be binned to arbitrary scales or plotted against the microscope tissue image. |
  | lab_processed/spaceranger/barcode_mappings.parquet$ |  | Contains spatial mapping information that links sequencing barcodes to physical locations and segmented cells. |
  | lab_processed/spaceranger/spatial/ | ✓ | Dataset level spatial files from the 10X Genomics SpaceRanger pipeline. |
  | lab_processed/spaceranger/spatial/tissue_hires_image.png$ | ✓ | PNG image of aligned tissue at full/original resolution |
  | lab_processed/spaceranger/spatial/tissue_lowres_image.png$ | ✓ | PNG image of aligned tissue at downscaled resolution |
  | lab_processed/spaceranger/spatial/cytassist_image.tiff$ | ✓ | Copy of the original CytAssist image input, which is a eosin-stained brightfield image in TIFF format that contains the fiducial frame. |
  | lab_processed/spaceranger/spatial/aligned_fiducials.jpg$ | ✓ | File used to verify that fiducial alignment was successful. Fiducial spots detected by the fiducial alignment algorithm are highlighted in red. |
  | lab_processed/spaceranger/spatial/aligned_tissue_image.jpg$ | ✓ | Image capturing the results of the image registration by merging the registered images in a checkerboard pattern with adjacent squares alternating between the two images. |
  | lab_processed/spaceranger/spatial/detected_tissue_image.jpg$ | ✓ | This image has either the dimensions of the CytAssist image or of the tissue_hires_image.png, and shows the following: (1) Aligned fiducial spots as hollow circles which are red (Visium HD) or blue (Visium v1/v2); (2) A blue bounding box. The interior designates the portion of the image where tissue is discriminated from background; (3) Spots/squares found under tissue are blue (Visium HD) or red (Visium v1/v2); (4) Spots/squares outside of tissue are gray. |
  | lab_processed/spaceranger/binned_outputs/ | ✓ | Binned outputs from the 10X Genomics SpaceRanger pipeline. |
  | lab_processed/spaceranger/binned_outputs/square_*/ | ✓ | Square-specific binned outputs from the 10X Genomics SpaceRanger pipeline. |
  | lab_processed/spaceranger/binned_outputs/square_*/filtered_feature_bc_matrix.h5$ | ✓ | HDF5 file containing binned called cell x gene matrix |
  | lab_processed/spaceranger/binned_outputs/square_*/raw_feature_bc_matrix.h5$ | ✓ | HDF5 file containing unfiltered binned cell x gene matrix |
  | lab_processed/spaceranger/binned_outputs/square_*/spatial/ | ✓ | Square-specific spatial outputs from the 10X Genomics SpaceRanger pipeline |
  | lab_processed/spaceranger/binned_outputs/square_*/spatial/aligned_fiducials.jpg$ | ✓ | Square-specific file used to verify that fiducial alignment was successful. Fiducial spots detected by the fiducial alignment algorithm are highlighted in red. |
  | lab_processed/spaceranger/binned_outputs/square_*/spatial/aligned_tissue_image.jpg$ | ✓ | Square-specific image capturing the results of the image registration by merging the registered images in a checkerboard pattern with adjacent squares alternating between the two images. |
  | lab_processed/spaceranger/binned_outputs/square_*/spatial/detected_tissue_image.jpg$ | ✓ | Square-specific image displaying the following: (1) Aligned fiducial spots as hollow circles which are red (Visium HD) or blue (Visium v1/v2); (2) A blue bounding box. The interior designates the portion of the image where tissue is discriminated from background; (3) Spots/squares found under tissue are blue (Visium HD) or red (Visium v1/v2); (4) Spots/squares outside of tissue are gray. |
  | lab_processed/spaceranger/binned_outputs/square_*/spatial/tissue_positions.parquet$ | ✓ | File ontains a table with rows that correspond to spots with columns corresponding to the following field: barcode, in_tissue, array_row, array_col, pxl_row_in_fullres and pxl_col_in_fullres. |
  | lab_processed/spaceranger/binned_outputs/square_*/spatial/scalefactors_json.json$ | ✓ | Spaceranger downscales images to 2000px on the smallest side.  This file contains the spot size (diameter in pixels) at full and downscaled resolutions. |
  | lab_processed/spaceranger/segmented_outputs/ | ✓ | Segmented output from the 10X Genomics SpaceRanger pipeline. |
  | lab_processed/spaceranger/segmented_outputs/cell_segmentations.geojson$ |  | Contains a spatial collection of polygons that define the exact physical boundaries of individual cells in a tissue sample. |
  | lab_processed/spaceranger/segmented_outputs/nucleus_segmentations.geojson$ |  | Contains spatial mapping output containing polygon coordinates that outline the exact boundaries of individual cell nuclei detected within a tissue sample. |
  | lab_processed/spaceranger/segmented_outputs/filtered_feature_bc_matrix.h5$ |  | HDF5 file that stores the quantified, tissue-associated gene (or feature) expression data for a spatial transcriptomics experiment. It acts as the primary input for downstream spatial analysis in tools like Seurat or spatialGE. |
  | lab_processed/spaceranger/segmented_outputs/spatial/ | ✓ | Segmented spatial output from the 10X Genomics SpaceRanger pipeline. |
  | lab_processed/spaceranger/segmented_outputs/spatial/scalefactors_json.json$ |  | Contains the scaling and resizing parameters needed to map transcriptomic data onto tissue microscopy images. It provides the exact multipliers and spot dimensions required to align spot barcodes with image pixels. |
